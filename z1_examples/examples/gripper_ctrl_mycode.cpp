...
int main(int argc, char** argv)
{
  ...
  if (std::string(argv[1]) == "k") {
    ...
    ROS_INFO("Use ←→ arrows to open/close, '-'/=' for speed, ','/'.' for force");
    ...
    while (ros::ok()) {
      ...
      // 调整步长（速度）
      if (key == '=' || key == '+' || key == 3) step_size = std::min(step_size * 1.5, 0.15);
      if (key == '-' || key == '_' || key == 4) step_size = std::max(step_size * 0.8, 0.02);
      
      // 调整力矩
      if (key == '.') max_effort = std::min(max_effort + 1.0, 20.0);
      if (key == ',') max_effort = std::max(max_effort - 1.0, 3.0);
      
      // 方向控制
      if (key == 2) current_target += step_size;  // 左键：打开夹爪
      if (key == 1) current_target -= step_size;  // 右键：闭合夹爪
