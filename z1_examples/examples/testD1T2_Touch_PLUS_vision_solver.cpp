...
if (it != ctrl_mode_map.end()) {
  ...
  // ==== 话题初始化 ====
  ros::Subscriber pose_sub = nh.subscribe("/touch2/pose", 10, poseCallback);
  ...
  ros::Subscriber sub = nh.subscribe("/touch2/Touch_Anchor_updated", 1, AnchorboolCallback);
  ros::Subscriber back_to_home_sub = nh.subscribe("/touch2/back_to_home_arm", 1, BackToHomeboolCallback);
  ...
  ros::Subscriber increment_sub = nh.subscribe("/touch2/Touch_Pose_increment", 10, poseIncrementCallback);
  ros::Subscriber sub_Touch_button = nh.subscribe("/touch2/joy", 1, joyCallback);
