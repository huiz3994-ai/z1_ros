...
double K_touch_pos_FIXED = 0.001;//映射参数，不建议调整
...
double K_touch_ori_CTRL = 0.2;//暂不起作用
...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr &msg){
    ...
    case only_touch_HR:
        ...
        processed_pose.pose.position.x += K_touch_pos_FIXED * K_touch_pos_CTRL * (-delta_z_received);
        processed_pose.pose.position.y += K_touch_pos_FIXED * K_touch_pos_CTRL * (delta_y_received);
        processed_pose.pose.position.z += K_touch_pos_FIXED * K_touch_pos_CTRL * (delta_x_received);
...
// ====== 替换/修改 orientIncrementCallback：在接收阶段先进行模式滤波（或延后在合成前也可） ======
