...
double K_touch = 0.3;  // Touch 映射缩放系数 (最大化空间利用率建议取 0.6)
...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {//Touch 映射回调函数
    ...
    z1 机械臂     Touch
        ...
        z   ————   y
    *******************/
    /*****将 Touch 的末端位置信号缩小 K_touch*100 倍，并相对锚点位置移动（默认相对forward=[0.25,0,0.43]位置控制）*****/
    // processed_pose.pose.position.x = 0.25;
    // processed_pose.pose.position.y = 0;
    // processed_pose.pose.position.z = 0.43;
    // processed_pose.pose.position.x += K_touch*(-msg->pose.position.z)/100; 
    // processed_pose.pose.position.y += K_touch*(-msg->pose.position.x)/100;
    // processed_pose.pose.position.z += K_touch*(msg->pose.position.y)/100; 

    /******（动态映射）******/
    processed_pose.pose.position.x = x_Anchor_arm_received;//0.25;
    processed_pose.pose.position.y = y_Anchor_arm_received;//0;
    processed_pose.pose.position.z = z_Anchor_arm_received;//0.43;
    processed_pose.pose.position.x += K_touch*(-delta_z_received)/100; 
    processed_pose.pose.position.y += K_touch*(-delta_x_received)/100;
    processed_pose.pose.position.z += K_touch*(delta_y_received)/100; 
