...
int main(int argc, char** argv)
{
  ...
  if (std::string(argv[1]) == "k") {
    ...
    ROS_INFO("Use ←→ arrows to open/close, '-'/=' for speed, ','/'.' for force, q to quit");
    ...
    while (ros::ok()) {
      ...
      // 调整步长（速度）
      if (key == '=' || key == '+') step_size = std::min(step_size * 1.5, 0.15);
      if (key == '-' || key == '_') step_size = std::max(step_size * 0.8, 0.02);
