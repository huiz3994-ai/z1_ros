 ...
 * *******************************************/
/*补充说明：
0.本代码基于 test3_real_time_ik_solver.cpp，实现纯视觉伺服控制，追踪 aruco 标记。
...
double K_ori = 2;   // Touch 末端姿态映射缩放系数，由于是绝对映射，目前无法缩放，直接修改会导致不可解
...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {//Touch 映射回调函数
    ...
    // 原始Aruco姿态数据
    geometry_msgs::PoseStamped aruco_pose = *msg;
    // ========== 位置映射部分 ========== 
    //坐标轴变换
    /******************
    z1 机械臂    aruco    Touch
    ...
    *******************/
    ...
    processed_pose.pose.position.x += K_pos*(msg->pose.position.z-0.4); 
    ...
    // if (0) { // 按键按下时更新姿态
        // processed_pose.pose.orientation.x = K_ori*(msg->pose.orientation.z);
        // processed_pose.pose.orientation.y = K_ori*(-msg->pose.orientation.x);
        // processed_pose.pose.orientation.z = K_ori*(-msg->pose.orientation.y);
        // processed_pose.pose.orientation.w = K_ori*(msg->pose.orientation.w);
