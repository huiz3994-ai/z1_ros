 ...
 * *******************************************/
/*补充说明：
0.本代码基于 test1_Touch_PLUS_vision_solver.cpp，实现Touch 主导控制的竖装映射。
...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {//Touch 映射回调函数
    ...
    case only_touch:
        ...
        // ========== 姿态映射部分 ==========
        // 使用[Touch]真实姿态映射 + x轴 -90 度偏转
        if (last_white_state)
        { // 按键按下时更新姿态（先按你原来的轴映射，再叠加固定偏转）
            // --- 先做你原有的分量映射（保持兼容现有逻辑） ---
            geometry_msgs::Quaternion mapped_q;
            mapped_q.x = K_touch_ori_FIXED * (-msg->pose.orientation.z);
            mapped_q.y = K_touch_ori_FIXED * (msg->pose.orientation.y);
            mapped_q.z = K_touch_ori_FIXED * (msg->pose.orientation.x);
            mapped_q.w = K_touch_ori_FIXED * (msg->pose.orientation.w);

            // --- 把映射后的四元数转换为 tf2 格式 ---
            tf2::Quaternion q_mapped;
            tf2::fromMsg(mapped_q, q_mapped);

            // --- 构造绕机械臂 x 轴逆时针 90° 的偏转四元数 ---
            // 注意：setRotation(axis, angle) 中 angle 单位为弧度
            const double half_pi = M_PI / 2.0;
            tf2::Quaternion q_offset;
            q_offset.setRotation(tf2::Vector3(1.0, 0.0, 0.0), -half_pi); // 绕 x 轴 -90°

            // --- 组合四元数：局部 x 轴偏转（在末端坐标系中）请使用 q_new = q_mapped * q_offset;
            // 如果你想的是全局基座系上的偏转，则改为 q_new = q_offset * q_mapped;
            tf2::Quaternion q_new = q_mapped * q_offset;

            q_new.normalize(); // 保证单位四元数

            // 写回 processed_pose，并记录为 last_orientation
            processed_pose.pose.orientation = tf2::toMsg(q_new);
            last_orientation = processed_pose.pose.orientation; // 保存用于松手时保持姿态
        }
        else
        { // 松开时使用最后一次记录的姿态（此处 last_orientation 已经包含偏转）
