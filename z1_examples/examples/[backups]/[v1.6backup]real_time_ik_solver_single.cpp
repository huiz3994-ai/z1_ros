...
void joyCallback(const sensor_msgs::Joy::ConstPtr& msg)
{
    ...
    if(msg->buttons.size() < 2) {
        ROS_WARN_ONCE("[ERROR]Invalid buttons message size!");
        return;
    }
    ...
    if(current_grey != last_grey_state) {
        if(current_grey == 1) {
            ROS_INFO_STREAM("[STATE]Gray button PRESSED");
            // 这里添加按下处理代码
        } else {
            ROS_INFO_STREAM("[STATE]Gray button RELEASED");
        }
    ...
    if(current_white != last_white_state) {
        if(current_white == 1) {
            ROS_INFO_STREAM("[STATE]White button PRESSED"); 
            // 这里添加按下处理代码
        } else {
            ROS_INFO_STREAM("[STATE]White button RELEASED");
        }
...
int main(int argc, char** argv) {
    ...
    while (ros::ok()) {
        if (new_target_pose_received) {
            ...
            // ros::Time start_time = ros::Time::now(); //[解算时间]开始记录点
            ...
            if (ik_solved) {
                ...
                // ros::Time end_time = ros::Time::now();   //[解算时间]结束记录点
