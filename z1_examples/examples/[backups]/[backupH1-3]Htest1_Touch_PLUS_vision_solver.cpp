...
// 新增：接收 Touch 发布的姿态相对四元数 [x,y,z,w]
void orientIncrementCallback(const std_msgs::Float64MultiArray::ConstPtr &msg)
{
    if (msg->data.size() >= 4)
    {
        delta_orient_received.x = msg->data[0];
        delta_orient_received.y = msg->data[1];
        delta_orient_received.z = msg->data[2];
        delta_orient_received.w = msg->data[3];

        // 归一化并校验
        tf2::Quaternion qtmp(delta_orient_received.x, delta_orient_received.y,
                             delta_orient_received.z, delta_orient_received.w);
        if (!std::isfinite(qtmp.x()) || !std::isfinite(qtmp.y()) ||
            !std::isfinite(qtmp.z()) || !std::isfinite(qtmp.w()))
        {
            ROS_WARN_STREAM_THROTTLE(1.0, "[WARN] Received invalid orient delta (NaN/Inf). Ignoring.");
            // 保持上一次的 delta_orient_received（通常是单位四元数）
            return;
        }
        qtmp.normalize();
        delta_orient_received.x = qtmp.x();
        delta_orient_received.y = qtmp.y();
        delta_orient_received.z = qtmp.z();
        delta_orient_received.w = qtmp.w();
    }
}
