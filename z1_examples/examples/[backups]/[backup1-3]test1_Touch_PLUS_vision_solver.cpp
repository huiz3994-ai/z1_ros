...
double K_touch_pos_FIXED = 0.1; // Touch 末端位置映射缩放系数 (最大化空间利用率建议取 0.6，正常灵敏度 0.1 or 0.2，稳定慢速灵敏度 0.05)
...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {//Touch 映射回调函数
    ...
    case only_touch:
        ...
        processed_pose.pose.position.x += K_touch_pos_FIXED * (-delta_z_received) / 100;
        processed_pose.pose.position.y += K_touch_pos_FIXED * (-delta_x_received) / 100;
        processed_pose.pose.position.z += K_touch_pos_FIXED * (delta_y_received) / 100;
        // ========== 姿态映射部分 ==========
        // 使用[Touch]真实姿态映射
