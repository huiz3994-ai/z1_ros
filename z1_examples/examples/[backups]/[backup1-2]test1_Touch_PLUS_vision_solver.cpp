...
double K_touch_pos = 0.1; // Touch 末端位置映射缩放系数 (最大化空间利用率建议取 0.6，正常灵敏度 0.1 or 0.2，稳定慢速灵敏度 0.05)
...
***/
    ...
    */
    ...
    if (last_white_state) { // 按键按下时更新姿态
        ...
        // aruco marker 姿态覆盖
        processed_pose.pose.orientation.x = K_aruco_ori * processed_aruco_marker_pose.pose.orientation.x;
        processed_pose.pose.orientation.y = K_aruco_ori * processed_aruco_marker_pose.pose.orientation.y;
        processed_pose.pose.orientation.z = K_aruco_ori * processed_aruco_marker_pose.pose.orientation.z;
        processed_pose.pose.orientation.w = K_aruco_ori * processed_aruco_marker_pose.pose.orientation.w;
