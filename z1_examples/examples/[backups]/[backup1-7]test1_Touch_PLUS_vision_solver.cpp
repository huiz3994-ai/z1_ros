...
#include <map>     // 用于参数映射
#include <std_msgs/Float64.h>//（debug用）中间数据发布
...
ros::Publisher k_pub;//（debug用）中间数据发布
ros::Publisher current_pose_pub;//（debug用）中间数据发布
...
void computeTargetPoseFromTouch() {
    ...
    case mixed_pos:
    {
        ...
        const float smoothing = 0.995f; // 平滑系数（0.95~0.999，越接近1过渡越慢）
...
if (it != ctrl_mode_map.end()) {
  ...
  k_pub = nh.advertise<std_msgs::Float64>("/debug/k_aruco_pos_ctrl", 10);//（debug用）中间数据发布
  current_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/debug/current_pose", 10);//（debug用）中间数据发布
  ...
  while (ros::ok() && !exit_flag)
  {
    ...
    if (new_target_pose_received)
    {
      ...
      current_pose_pub.publish(current_pose);//（debug用）中间数据发布
      // ==== 逆运动学解算 ====
