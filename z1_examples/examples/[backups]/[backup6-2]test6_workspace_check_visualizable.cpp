...
#include <random>
#include <limits>  // 新增头文件处理四元数范围
/***程序运行结果（机械臂末端笛卡尔空间坐标取值范围）（一万样本）
[位置描述]
X Range: [-0.659, 0.668]
Y Range: [-0.657, 0.678]
Z Range: [-0.167, 0.797]
[姿态描述（四元数）]
Quaternion W Range: [-0.500, 0.999]
Quaternion X Range: [-0.857, 0.997]
Quaternion Y Range: [-0.848, 0.996]
Quaternion Z Range: [-0.859, 0.997]
...
int main(int argc, char** argv) {
    ...
    double max_w = std::numeric_limits<double>::lowest();
    ...
    // 输出四元数范围（仅在有效样本存在时）
    if (!cloud.empty()) {
        ROS_INFO("Quaternion W Range: [%.3f, %.3f]", min_w, max_w);
        ROS_INFO("Quaternion X Range: [%.3f, %.3f]", min_x, max_x);
        ROS_INFO("Quaternion Y Range: [%.3f, %.3f]", min_y, max_y);
        ROS_INFO("Quaternion Z Range: [%.3f, %.3f]", min_z, max_z);
    }
